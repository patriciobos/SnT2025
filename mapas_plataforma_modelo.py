...
# === Alias para los títulos de mapas de puntos ===
ALIAS_PUNTOS = {
    "zais": "MDQ",   # podés cambiar a lo que quieras
    "gsj":  "CRD",    # podés cambiar a lo que quieras
}

# Alias SOLO para mostrar en título y leyenda
ALIAS_ZONA = {
    "zais": "MDQ",   # si ZONA == 'zais' → mostrar 'MDQ'
    "gsj":  "CRD",   # podés agregar más, ej: "gsj": "GSJ"
}
...
# ==== MAPAS DE PUNTOS (SIN INTERPOLACIÓN) PARA GSJ Y ZAIS ====

FRECUENCIA_FIJA_PUNTOS = 100.0   # como acordamos
PUNTOS_MARKER = 'o'
PUNTOS_SIZE = 2
PUNTOS_EDGE_LW = 0.1

def _aplicar_exclusiones_csv_en_puntos(df):
    """Aplica exclusiones por lista (excluir.csv) sobre filas del DataFrame."""
    if EXCLUSIONES is None or len(EXCLUSIONES) == 0 or len(df) == 0:
        return df
    ...
    if np.any(mask_excluir):
        print(f"[INFO] (puntos) Excluidas {int(mask_excluir.sum())}/{len(df)} filas por excluir.csv")
    return df[~mask_excluir].copy()

def _mascara_geo_punto(lat, lon):
    """True si pasa la máscara geográfica compuesta (círculo GSJ, plataforma a la izquierda de corte)."""
    # círculo GSJ
    ok_circulo = _dist_km_to(lat, lon, GSJ_LAT, GSJ_LON) <= CIRCLE_R_KM

    # plataforma a la izquierda del meridiano de corte
    if lon <= MERIDIANO_CORTE:
        if PLATAFORMA_GEOM is not None:
            try:
                return ok_circulo and PLATAFORMA_GEOM.covers(Point(lon, lat))
            except Exception:
                return ok_circulo
        else:
            return ok_circulo
...
def _mask_sector_AB_punto(lat, lon, p_lat, p_lon):
    """True si el punto NO cae dentro del sector A–B (i.e., pasa el filtro)."""
    # ángulos A y B respecto al punto de zona
    ang_a = _azimuth(p_lat, p_lon, A_LAT, A_LON)
    ang_b = _azimuth(p_lat, p_lon, B_LAT, B_LON)
    # r_min según promedio de distancias a A y B
    dist_a = geodesic((p_lat, p_lon), (A_LAT, A_LON)).km
    dist_b = geodesic((p_lat, p_lon), (B_LAT, B_LON)).km
    r_min_km = (dist_a + dist_b) / 2.0

    ang_pt = _azimuth(p_lat, p_lon, lat, lon)
    r_pt   = _dist_km_to(lat, lon, p_lat, p_lon)

    in_annulus = (r_pt >= r_min_km) & (r_pt <= R_MAX_KM)
    in_sector  = _ang_in_short_arc(ang_pt, ang_a, ang_b)
    remove = in_annulus & in_sector
    return ~remove  # True si pasa (no pertenece al sector de exclusión)

def _recolectar_datos_filtrados_por_zona(zona: str) -> pd.DataFrame:
    """Lee todos los CSV de la zona, filtra frecuencia fija y aplica máscaras geo + sector A–B."""
    archivos = [
        os.path.join(CARPETA_INPUT, f)
        for f in os.listdir(CARPETA_INPUT)
        if f.endswith(".csv") and zona in f
    ]
    if not archivos:
        print(f"[WARN] No se encontraron CSV para zona {zona} en {CARPETA_INPUT}")
        return pd.DataFrame(columns=['lat', 'lon', VAR_TL, 'bat'])
    ...
    for ruta in archivos:
        ...
        # filtros básicos (sin excluir.csv, sin TL <200)
        df = df[df[VAR_TL] > FILTRO_TL_MIN]
        df = df[df['bat'] > 30]

        dfs.append(df[['lat', 'lon', VAR_TL, 'bat']])

    if not dfs:
        print(f"[WARN] No hay datos con f={FRECUENCIA_FIJA_PUNTOS} Hz para zona {zona}.")
        return pd.DataFrame(columns=['lat', 'lon', VAR_TL, 'bat'])
    ...
    if p_lon is not None and p_lat is not None and len(datos) > 0:
        ...
        mask_geo = np.array([_mascara_geo_punto(lt, ln) for lt, ln in zip(lat_arr, lon_arr)], dtype=bool)
        mask_sector = np.array([_mask_sector_AB_punto(lt, ln, p_lat, p_lon) for lt, ln in zip(lat_arr, lon_arr)], dtype=bool)

        mask_final = mask_geo & mask_sector
        if not np.all(mask_final):
            print(f"[INFO] (puntos) {int((~mask_final).sum())} filas removidas por geo/sector en {zona}")
        datos = datos[mask_final].copy()

    return datos


def _dibujar_arcos_exclusion(ax, m, p_lat, p_lon):
    """Dibuja los arcos y radios del sector A–B si corresponde."""
    ang_a = _azimuth(p_lat, p_lon, A_LAT, A_LON)
    ang_b = _azimuth(p_lat, p_lon, B_LAT, B_LON)
    dist_a = geodesic((p_lat, p_lon), (A_LAT, A_LON)).km
    dist_b = geodesic((p_lat, p_lon), (B_LAT, B_LON)).km
    ...
    # arco exterior
    arco_max = [geodesic(kilometers=R_MAX_KM).destination((p_lat, p_lon), ang) for ang in angles]
    x_max, y_max = m([d.longitude for d in arco_max], [d.latitude for d in arco_max])
    ax.plot(x_max, y_max, linestyle='--', linewidth=1.2, color='red')
    # arco interior
    arco_min = [geodesic(kilometers=r_min_km).destination((p_lat, p_lon), ang) for ang in angles]
    x_min, y_min = m([d.longitude for d in arco_min], [d.latitude for d in arco_min])
    ax.plot(x_min, y_min, linestyle='--', linewidth=1.2, color='red')
    # radios
    p_a_min = geodesic(kilometers=r_min_km).destination((p_lat, p_lon), ang_a)
    p_a_max = geodesic(kilometers=R_MAX_KM).destination((p_lat, p_lon), ang_a)
    x_ra, y_ra = m([p_a_min.longitude, p_a_max.longitude], [p_a_min.latitude, p_a_max.latitude])
    ax.plot(x_ra, y_ra, linestyle='--', linewidth=1.2, color='red')

    p_b_min = geodesic(kilometers=r_min_km).destination((p_lat, p_lon), ang_b)
    p_b_max = geodesic(kilometers=R_MAX_KM).destination((p_lat, p_lon), ang_b)
    x_rb, y_rb = m([p_b_min.longitude, p_b_max.longitude], [p_b_min.latitude, p_b_max.latitude])
    ax.plot(x_rb, y_rb, linestyle='--', linewidth=1.2, color='red')

def generar_mapa_puntos(zona: str):
    """Crea un mapa sin interpolación con puntos filtrados para la zona dada."""
    display_zona = nombre_para_mostrar(zona)
    alias_puntos = ALIAS_PUNTOS.get(zona.lower(), zona.upper())
    datos = _recolectar_datos_filtrados_por_zona(zona)

    # FIGURA/BASEMAP con límites fijos (reutiliza tu estilo actual)
    fig, ax = plt.subplots(figsize=(10, 8))
    m = Basemap(
        projection='merc',
        llcrnrlat=MAP_LL_LAT, urcrnrlat=MAP_UR_LAT,
    ...
    # contorno SHP plataforma (con handle para la leyenda)
    try:
        shelf_handle = ax.plot([], [], color='gray', linewidth=1.5, label='Argentine Continental Shelf')[0]
        gdf_mascara = gpd.read_file(MASCARA_SHP).to_crs("EPSG:4326")
        for geom in gdf_mascara.geometry:
            if geom.geom_type == 'Polygon':
                x, y = m(*geom.exterior.xy); ax.plot(x, y, color='gray', linewidth=1.5)
                for ring in geom.interiors:
                    xi, yi = m(*ring.xy); ax.plot(xi, yi, color='gray', linewidth=1.0)
            elif geom.geom_type == 'MultiPolygon':
                for poly in geom.geoms:
                    x, y = m(*poly.exterior.xy); ax.plot(x, y, color='gray', linewidth=1.5)
                    for ring in poly.interiors:
                        xi, yi = m(*ring.xy); ax.plot(xi, yi, color='gray', linewidth=1.0)
    except Exception as e:
        print(f"[WARN] No se pudo dibujar el SHP de plataforma: {e}")
    ...
    # scatter de puntos (primero, zorder más bajo)
    points_handle = None
    if len(datos) > 0:
        xx, yy = m(datos['lon'].to_numpy(), datos['lat'].to_numpy())
        points_handle = ax.scatter(
            xx, yy, s=PUNTOS_SIZE, marker=PUNTOS_MARKER,
            facecolors='gray', edgecolors='black', linewidths=PUNTOS_EDGE_LW,
    ...
    if PLOT_EXCLUSION_ARCS and (p_lon is not None) and (p_lat is not None):
        _dibujar_arcos_exclusion(ax, m, p_lat, p_lon)

    # leyenda: buoy primero, luego puntos, luego shelf
    handles = []
    ...
    # título y guardado igual que antes
    plt.title(
        f"N geodesic radial lines from the buoy for TL modeling",
        fontsize=16
    )
    os.makedirs(CARPETA_OUTPUT, exist_ok=True)
    out_path = os.path.join(CARPETA_OUTPUT, f"{zona.upper()}_puntos.png")
    plt.savefig(out_path, dpi=300, bbox_inches="tight")
    plt.close()
    print(f"[OK] Mapa de puntos guardado → {out_path}")

# ==== EJECUCIÓN DE LOS DOS MAPAS (GSJ y ZAIS) ====
def generar_mapas_puntos_gsj_zais():
    for zona in ["gsj", "zais"]:
        generar_mapa_puntos(zona)

# Llamalo al final del main si querés que se ejecute automáticamente:
# generar_mapas_puntos_gsj_zais()
